...
// OK - implements exactly what's specified
...
// Not OK - adds unspecified functionality
export class UserService {
  async getById(id: string): Promise<User> { ... }
...
# Not OK - deviates from spec signature
...
- OK **Always**: Follow spec exactly, use provided code examples, implement all specified items, mark todos complete
- Ask **Ask first**: If spec is ambiguous or incomplete, if implementation conflicts with existing code
- Never **Never**: Deviate from spec, add unspecified features, make architectural decisions, modify tests, run test suites, research alternatives
...
```txt
OK Created src/types/result.ts
OK Implemented Success<T> type
OK Implemented Failure<E> type
OK Implemented Result<T, E> union
OK Implemented success() helper
OK Implemented failure() helper
